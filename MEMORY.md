...
## Obsidian 知识库
- **位置**：`D:\ClawX-Data\Obsidian\uav_gpr\`
- **状态**：2026-04-15 结构重置完成，已接管
- **2026-04-15 算法卡恢复记录**：
  - 事故：用 `sed` 批量修复链接时，Windows bash 下的 sed 不支持 UTF-8，导致 `20-数据处理/20-算法卡/` 下 9 个文件损坏
  - 恢复结果：
    - `零时矫正.md`、`背景抑制.md`、`增益.md`：凭完整阅读记忆重写
    - `低频漂移矫正.md`、`尖锐杂波抑制.md`：从损坏文件中提取前 8-9 节，补全链接
    - `去噪.md`、`频域滤波.md`、`迁移成像.md`、`时间-深度转换.md`：凭 frontmatter + warning 字典 + 决策入口中的引用信息重建
  - 后续教训：处理中文 UTF-8 文件禁用 sed，统一用 Python
- **2026-04-15 内部链接修复**：
  - 修复了整个 vault 中所有 `[[03-数据处理/...]]`、`[[07-案例库/...]]`、`[[01-项目/...]]`、`[[02-数据采集/...]]`、`[[05-质量控制/...]]` 等旧路径链接
  - 全部指向新编号结构（20-数据处理/、30-案例与证据/、10-项目/、80-采集与解释/ 等）
- **2026-04-15 知识地图修复**：
  - 修复了 `50-知识地图/canvas/` 下 3 个 `.canvas` 文件中的旧路径节点引用
  - 修复了 `知识地图总览.md` 中的 canvas 嵌入路径（补充 `canvas/` 子目录）
- **新结构**：
  - `00-首页与导航/` — 欢迎页 + 工作流入口
  - `10-项目/` — 项目卡、真实数据验证、长期规则
  - `20-数据处理/` — 决策入口、参数评估、算法卡
  - `30-案例与证据/` — 案例库、按测线归集的验证截图
  - `40-归档与历史/` — 版本归档索引 + 简化文件名快照
  - `50-知识地图/` — Canvas 知识地图
  - `60-数据与资源/` — 统一数据库(base) + 文献资料
  - `70-模板/` — 项目/算法/案例/验证/复盘模板
  - `80-采集与解释/` — 轻量预留分区
  - `99-附件/` — 零散图片
